...
class DoGFilter(nn.Module):
    def __init__(self, in_channels, kernel_size,sigma1,sigma2, stride=1, padding=(2,2)):
        ...
        self.sigma2 = sigma2
        # initiate
        self.conv1 = nn.Conv2d(in_channels=1,out_channels=1,kernel_size=5,padding=self.padding,bias = False)
        self.conv2 = nn.Conv2d(in_channels=1,out_channels=1,kernel_size=5,padding=self.padding,bias=False)
    ...
    def DoG_kernel(self, s1, s2, size):
        ...
        g1 = torch.exp(-(xx**2 + yy**2) / (2 * s1**2))
        g1 = g1/g1.sum()
        g2 = torch.exp(-(xx**2 + yy**2) / (2 * s2**2))
        g2 = g2/g2.sum()
        # transfer to Tensor
        g1 = g1.view(1,1,5,5)
        g2 = g2.view(1,1,5,5)
